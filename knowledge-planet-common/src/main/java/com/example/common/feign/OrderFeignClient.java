...
public interface OrderFeignClient {
    ...
    Result<OrderVO> createOrder(@RequestBody OrderCreateDTO orderCreateDTO, @RequestHeader("Authorization") String token);

    @GetMapping("/order/check/{userId}/{planetId}")
    Result<Boolean> checkUserPlanetOrder(@PathVariable("userId") Long userId, @PathVariable("planetId") Long planetId);
}